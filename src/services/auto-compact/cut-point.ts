...
export function calculateCutPoint(
  ...
  const minMessagesToKeep = Math.min(
...
export function validateCutPoint(
  ...
  if (result.messagesToKeep < config.preserveRecentTurns) {
    return {
      valid: false,
      reason: `Cut point would keep only ${result.messagesToKeep} messages, ` +
  ...
  if (result.tokensAfterCut > config.maxTokensToKeep) {
    return {
      valid: false,
      reason: `Tokens after cut (${result.tokensAfterCut}) exceeds max ` +
              `(${config.maxTokensToKeep})`
    };
  }
  ...
  if (result.tokensAfterCut < config.minTokensToKeep && result.messagesToCompact > 0) {
    return {
      valid: false,
      reason: `Tokens after cut (${result.tokensAfterCut}) below min ` +
              `(${config.minTokensToKeep})`
    };
  }
