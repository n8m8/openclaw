...
import { calculateTokenBudget, shouldTriggerCompaction, getWarningLevel, generateWarningMessage } from "./token-budget.js";
import { executeCompaction, applyCompaction, createHistoryEntry, formatCompactionNotification, isCompactionWorthwhile } from "./executor.js";
...
export async function checkAutoCompaction(
  ...
  const shouldCompact = options?.force || 
    (config.enabled && shouldTriggerCompaction(budget, config.triggerPercentage));
  ...
  if (!options?.force && !isCompactionWorthwhile(session.messages, config)) {
    ...
    return {
      messages: session.messages,
      compacted: false,
      notification: notification || "⚠️ Token limit approaching but compaction wouldn't help. Consider /continue to start fresh.",
      metadata,
      budget
    };
  }
  ...
  try {
    ...
    if (!result.compacted) {
      ...
      return {
        messages: session.messages,
        compacted: false,
        notification: "⚠️ Auto-compaction attempted but couldn't compress session. Consider /continue.",
        metadata,
        budget
      };
    }
...
export function getCompactionStats(metadata: SessionCompactionMetadata): {
  ...
  return {
    ...
    averageTokensReclaimed: metadata.autoCompactCount > 0
      ? Math.round(totalTokensReclaimed / metadata.autoCompactCount)
      : 0,
    lastCompactedAt: metadata.lastAutoCompactAt
      ? new Date(metadata.lastAutoCompactAt)
      : undefined
  };
}
...
export function formatCompactionStats(metadata: SessionCompactionMetadata): string {
  ...
  const lastCompacted = stats.lastCompactedAt
    ? stats.lastCompactedAt.toLocaleString()
    : "never";
