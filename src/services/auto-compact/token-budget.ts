...
export function calculateTokenBudget(
  ...
  const averageTokensPerTurn = turnCount > 0 
    ? Math.ceil(used / turnCount)
    : DEFAULT_AVG_TOKENS_PER_TURN;

  const estimatedTurnsLeft = averageTokensPerTurn > 0
    ? Math.floor(remaining / averageTokensPerTurn)
    : 0;
 ...
 */
export function getWarningLevel(budget: TokenBudget): 'none' | 'low' | 'medium' | 'high' | 'critical' {
